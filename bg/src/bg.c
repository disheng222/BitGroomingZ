...
{
	return BG_compress(dataType, data, outSize, confparams_cpr->bgMode, confparams_cpr->NSD, confparams_cpr->DSD, nbEle);
}
...
unsigned char* BG_compress_args(int dataType, void *data, size_t *outSize, int bgMode, int errorControlMode, int nsd, int dsd, size_t nbEle)
{
	...
	size_t bufferSize = ataTypeLen*nbEle;
	...
	if(errorControlMode == NSD && (nsd < 0 || nsd >16))
	{
	...
	if(!(type == NC_FLOAT  && prc_bnr_xpl_rqr >= bit_xpl_nbr_sgn_flt) || (type == NC_DOUBLE && prc_bnr_xpl_rqr >= bit_xpl_nbr_sgn_dbl)) //required # bits is greater than the full length of bits	
	{
